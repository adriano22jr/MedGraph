"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Line3D = void 0;
const g_1 = require("@antv/g");
const g6_1 = require("@antv/g6");
class Line3D extends g6_1.BaseEdge {
    getKeyPath() {
        return [];
    }
    getKeyStyle(attributes) {
        const { sourceNode, targetNode } = this;
        const [x1, y1, z1] = sourceNode.getPosition();
        const [x2, y2, z2] = targetNode.getPosition();
        // omit path
        const { d, ...style } = super.getKeyStyle(attributes);
        return { x1, y1, z1, x2, y2, z2, ...style };
    }
    drawKeyShape(attributes = this.parsedAttributes, container = this) {
        return this.upsert('key', g_1.Line, this.getKeyStyle(attributes), container);
    }
}
exports.Line3D = Line3D;
//# sourceMappingURL=line-3d.js.map