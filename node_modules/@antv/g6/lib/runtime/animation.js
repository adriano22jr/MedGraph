...
class Animation {
    ...
    animate(localAnimation, callbacks, extendOptions) {
        ...
        const animations = this.getTasks()
            ...
            cb?.before?.();
            const animation = (0, executor_1.executor)(element, this.inferStyle(context, extendOptions), options);
            if (animation) {
    ...
    inferStyle(context, options) {
        ...
        const getCacheOpacity = () => (0, cache_1.getCachedStyle)(element, 'opacity') ?? (0, animation_1.inferDefaultValue)('opacity');
        if (stage === 'enter') {
