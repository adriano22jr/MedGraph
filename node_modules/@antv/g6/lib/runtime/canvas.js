...
class Canvas {
    ...
    async init() {
        ...
        const names = ['main', 'label', 'transient', 'background'];
        const { renderers, canvas } = names.reduce((acc, name) => {
            const renderer = (0, util_1.isFunction)(getRenderer) ? getRenderer?.(name) : new g_canvas_1.Renderer();
            if (name === 'main') {
    ...
    getRendererType(layer = 'main') {
        const plugins = this.renderers[layer].getPlugins();
        for (const plugin of plugins) {
            if (plugin.name === 'canvas-renderer')
                return 'canvas';
            if (plugin.name === 'svg-renderer')
                return 'svg';
            if (plugin.name === 'device-renderer')
                return 'gpu';
        }
        return 'unknown';
    }
    ...
    setBackground(background = this.config.background) {
    ...
    getBounds() {
        return (0, bbox_1.getCombinedBBox)(Object.values(this.canvas)
    ...
    getContainer() {
        ...
        return (0, util_1.isString)(container) ? document.getElementById(container) : container;
    }
    appendChild(child) {
    ...
    async toDataURL(options = {}) {
        ...
        return new Promise((resolve) => {
            offscreenCanvas.on(g_1.CanvasEvent.RERENDER, async () => {
...
function destroyCanvas(canvas) {
    ...
    animationsWithPromises.forEach((animation) => {
        if (animation.effect.target)
            animation.effect.target = null;
        // @ts-expect-error private property
        if (animation.effect.computedTiming)
            animation.effect.computedTiming = null;
    });
    // @ts-expect-error private property
